...
public class MemberServiceImpl implements MemberService {
    ...
    @Override
    public String register(MemberSecurityDTO memberSecurityDTO) {

        Member member = modelMapper.map(memberSecurityDTO, Member.class);
        String mid = memberRepository.save(member).getMid();
        return "mid";
    }
    ...
    public MemberSecurityDTO readOne(String mid) {
        ...
        MemberSecurityDTO memberSecurityDTO = modelMapper.map(result, MemberSecurityDTO.class);
    ...
    @Override
    public void modify(MemberSecurityDTO memberSecurityDTO) /*throws MidExistException*/ {
        //기존에 id가 있는지 확인
        String mid = memberSecurityDTO.getMid(); // 프론트에서 id가 넘어옴
        String mpw = memberSecurityDTO.getMpw(); // 프론트에서 pw가 넘어옴

        Optional<Member> result = memberRepository.findById(mid);
        Member member = result.orElseThrow()/*->new MidExistException())*/;
