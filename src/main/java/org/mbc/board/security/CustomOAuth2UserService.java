...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    // p755 추가 (카톡 로그인 시 db에 같은 이메일이 있는지 처리)
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest)
        ...
        log.info("NAME : " + clientName); // 클라이언트 이름을 가져와 출력
...
/*      p751 제외
    ...
    private MemberSecurityDTO generateDTO(String email, Map<String, Object> params) {
        ...
        if(result.isEmpty()){
            ...
            Member member = Member.builder()
                    ...
                    .build();
            member.addRole(MemberRole.USER);   // 처음 로그인하면 일반사용자 롤
            memberRepository.save(member);      // db에 저장됨!

            //db에 저장 성공 후 프론트로 dto를 보냄
           ...
           return memberSecurityDTO;
            ...
            // 데이터베이스에 해당 이메일이 있으면!!!
            // 또 회원가입할 필요가 없다.
            Member member = result.get(); // member 엔티티 값을 가져와
            MemberSecurityDTO memberSecurityDTO = new MemberSecurityDTO(
                    member.getMid(),
                    member.getMpw(),
                    member.getEmail(),
    ...
    private String getKakaoEmail(Map<String, Object> paramMap) {
        log.info("CustomOAuth2UserService.getKakaoEmail에서 실행......");
        log.info("카카오 로그인 됨...... 이메일 주소 수집용......");

        Object value = paramMap.get("kakao_account");   // 키값을 가져와 value 처리

        log.info(value);
        //{profile_nickname_needs_agreement=false,
        // profile={nickname=김수아, is_default_nickname=false},
        // has_email=true, email_needs_agreement=false,
        // is_email_valid=true, is_email_verified=true,
        // email=gibong74@naver.com}

        LinkedHashMap accountMap = (LinkedHashMap) value;   // map 종류 중 하나

        String email = (String) accountMap.get("email");    // email=gibong74@naver.com}
        log.info(email);
